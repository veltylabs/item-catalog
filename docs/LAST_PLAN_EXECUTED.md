...
# Plan — `github.com/veltylabs/item_catalog`: a list stops claiming it has columns
...
**Files:** `model_orm.go` (14 list types).
...
3. Confirm the diff contains **only** removals of the two stub methods —
   14 `Schema()` and 14 `Pointers()` lines — and nothing else. If
...
| Stage | Files | Action |
|---|---|---|
| 1 | `model_orm.go` | regenerate with `ormc`; 14 stub pairs disappear |
